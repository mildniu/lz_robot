...
Write-Host "[1/3] Building PySide6 desktop app with PyInstaller..." -ForegroundColor Cyan
& $pythonExe -m PyInstaller --noconfirm --clean QuantumBotQt.spec
if ($LASTEXITCODE -ne 0) {
    throw "PyInstaller build failed with exit code: $LASTEXITCODE"
}

$distRoot = Join-Path $projectRoot "dist\QuantumBotQt"
$scriptsDist = Join-Path $distRoot "scripts"

Write-Host "[2/3] Creating scripts folder in final Qt dist..." -ForegroundColor Cyan
...
Write-Host "[3/3] Verifying packaged Qt scripts folder..." -ForegroundColor Cyan
Get-ChildItem -Force $scriptsDist | Select-Object Name, Length

Write-Host ""
Write-Host "Qt build complete:" -ForegroundColor Green
Write-Host "  EXE:     $distRoot\QuantumBotQt.exe"
